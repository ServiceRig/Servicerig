...
import { getEstimateTemplateById } from '@/lib/firestore/templates';
...
function EditEstimateTemplatePageContent({ templateId }: { templateId: string }) {
    ...
    const [state, formAction] = useActionState(updateEstimateTemplateAction, { success: false, message: null, errors: null });
    ...
    useEffect(() => {
        const fetchTemplate = async () => {
            setIsLoading(true);
            const fetchedTemplate = await getEstimateTemplateById(templateId);
            if (fetchedTemplate) {
                setTemplate(fetchedTemplate);
                setTitle(fetchedTemplate.title);
                setLineItems(fetchedTemplate.lineItems);
                if (fetchedTemplate.gbbTier) {
                    setGbbTier(fetchedTemplate.gbbTier);
                }
            }
            setIsLoading(false);
        }
    ...
    useEffect(() => {
        if (state?.success) {
            toast({
                title: 'Success',
                description: 'Estimate template updated successfully.',
            });
        ...
        <form action={formAction}>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold">Edit Estimate Template</h1>
                        <p className="text-muted-foreground">Modify the reusable template for common jobs.</p>
                    </div>
                    <SubmitButton />
                </div>
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <Card>
                            ...
                            <CardContent>
                                    ...
                                    <TableBody>
                                        {lineItems.map((item, index) => (
                                            <TableRow key={index}>
                                                ...
                                                <TableCell>
                                                    <Button variant="ghost" size="icon" type="button" onClick={() => handleRemoveLineItem(index)} disabled={lineItems.length <= 1}>
                                ...
                                <Button variant="link" type="button" onClick={handleAddLineItem} className="mt-4"><PlusCircle className="mr-2 h-4 w-4" />Add Item</Button>
                            </CardContent>
...
export default function EditEstimateTemplatePage({ params }: { params: Promise<{ templateId: string }> }) {
    const resolvedParams = use(params);
    return (
        <Suspense fallback={<div>Loading...</div>}>
            <EditEstimateTemplatePageContent templateId={resolvedParams.templateId} />
        </Suspense>
    )
}
